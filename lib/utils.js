...
async function updateTargetGeoStateAndHour (oldTarget, newTarget) {
  const { id: oldId, accept: oldAccept } = oldTarget
  const { id: newId, accept: newAccept } = newTarget

  const oldGeoStatePromises = oldAccept.geoState.$in.map(
    state => sremAsync(`geoState:${state}`, oldId)
  )

  const oldHourPromises = oldAccept.hour.$in.map(
    h => sremAsync(`hour:${h}`, oldId)
  )

  const newGeoStatePromises = newAccept.geoState.$in.map(
    state => saddAsync(`geoState:${state}`, newId)
  )

  const newHourPromises = newAccept.hour.$in.map(
    h => saddAsync(`hour:${h}`, newId)
  )
